...
def get_article_info(href: str) -> str:
    ...
    try:
        ...
        raise core.RequestErrorException('Invalid request', parent=e)
    # r = requests.get(href, headers=headers, cookies=cookies)
    try:
        r.raise_for_status()
    except Exception as e:
        raise core.RequestErrorException(f'Response error\nURL:{href}\nStatus:{r.status_code}\nReason:{r.reason}', parent=e)
    if not r.text.strip():
        raise core.RequestErrorException(f'Empty response body error')
    return r.text


def save_to_disk_old(article: core.ArticleInfo = None, file_name: str = '') -> None:
    try:
        if not article:
            raise core.SavingErrorException(f'File_name: {file_name}\nReason: Empty article')
        if not file_name:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with zipfile.ZipFile(os.path.join(core.ROOT_DIR, file_name), "w") as zpf:
...
def save_to_disk(article: core.ArticleInfo = None, file_name: str = '') -> None:
    try:
        if not article:
            raise core.SavingErrorException(f'File_name: {file_name}\nReason: Empty article')
        if not file_name:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with open(os.path.join(temp_dir, file_name), "rb") as arch, \
                 open(os.path.join(core.ROOT_DIR, file_name), "wb") as lzout:
...
def decompress_archive(file_name):
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            with open(os.path.join(core.ROOT_DIR, file_name), 'rb') as compressed,\
            ...
            with zipfile.ZipFile(os.path.join(temp_dir, file_name), "r") as fp:
                with open(os.path.join(core.ROOT_DIR, f'{file_name[:-3]}_article.html'), 'wb') as html_file, \
                    open(os.path.join(core.ROOT_DIR, f'{file_name[:-3]}_article.json'), 'wb') as json_file:
                    html_file.write(fp.read('article.html'))
                    json_file.write(fp.read('article.json'))
    except Exception as e:
        raise core.ReadingErrorException(f'Decompress error\nFile_name: {file_name}', parent=e)


def read_from_disk(file_name: str = '') -> core.ArticleInfo:
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            with open(os.path.join(core.ROOT_DIR, file_name), 'rb') as compressed, \
                    open(os.path.join(temp_dir, file_name), 'wb') as decompressed:
        ...
        raise core.ReadingErrorException(f'Read from file error\nFile_name: {file_name}', parent=e)


def get_last_pars_dt() -> datetime:
    try:
        with open(core.conf_last_parsing_dt_filename, 'r') as dtf:
...
def set_last_pars_dt():
    with open(core.conf_last_parsing_dt_filename, 'w') as f:
